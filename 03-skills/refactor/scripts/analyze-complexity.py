...
通过对比重构前后的指标，帮助衡量重构的影响。

用法：
    python analyze-complexity.py <文件>
    python analyze-complexity.py <重构前文件> <重构后文件>  # 对比模式
    python analyze-complexity.py --dir <目录>               # 分析目录

指标说明：
    - 圈复杂度（Cyclomatic Complexity）：代码中的决策点数量
    - 认知复杂度（Cognitive Complexity）：代码理解的困难程度
    - 可维护性指数（Maintainability Index）：整体可维护性评分（0-100）
    - 代码行数（Lines of Code）：总行数
    - 函数数量（Function Count）：函数/方法的数量
    - 平均函数长度（Average Function Length）：每个函数的平均行数
"""
...
class ComplexityAnalyzer:
    ...
    def calculate_cyclomatic_complexity(self, code: Optional[str] = None) -> int:
        """
        使用 McCabe 方法计算圈复杂度。
        公式：CC = E - N + 2P，其中 E=边数，N=节点数，P=连通分量数
        简化方法：统计决策点数量 + 1
        """
        if code is None:
            code = self.code
...
def print_metrics(metrics: FileMetrics, verbose: bool = False) -> None:
    ...
    print(f"  函数/方法数：           {metrics.function_count}")
    ...
    if verbose and metrics.functions:
        ...
        for f in sorted(metrics.functions, key=lambda x: x.cyclomatic_complexity, reverse=True):
            ...
            print(f"  {f.name}() [第 {f.start_line}-{f.end_line} 行]{flag}")
            print(f"    - 行数：{f.lines}，圈复杂度：{f.cyclomatic_complexity}，"
                  f"认知复杂度：{f.cognitive_complexity}，参数个数：{f.parameter_count}")
...
def print_comparison(before: FileMetrics, after: FileMetrics) -> None:
    ...
    print(f"\n  总结：{improvements} 项改善，{regressions} 项退步")
    print("=" * 70)
...
def analyze_directory(directory: str, verbose: bool = False) -> None:
    ...
    if not files:
        print(f"在 {directory} 中未找到受支持的文件")
        return
